...
//    if(NULL == rtask_NullDamp)  {return 1;}       //Error check
  ...
  long long iter = 0, cur_iter=0; double dt=0.00001;
  ...
  /******************************TRAJECTORY GENERATION************************************/
      ...
      for(int i = 0; i < dof; i++) IP->MaxVelocityVector->VecData[i] = 15;
      for(int i = 0; i < dof; i++) IP->MaxAccelerationVector->VecData[i] = 2;
...
//      while(!has_updated_dyn_after_robot_comm_)
      {
...
#pragma omp parallel private(thread_id)
  {
    ...
    if(thread_id==1) //Simulate physics and update the rio data structure..
    {
      ...
      for(int i = 0; i < dof; i++) IP->CurrentPositionVector->VecData[i] = pos(i);
      for(int i = 0; i < dof; i++) IP->CurrentVelocityVector->VecData[i] = 0;
      for(int i = 0; i < dof; i++) IP->CurrentAccelerationVector->VecData[i] = 0;

      for(int i = 0; i < dof; i++) OP->NewPositionVector->VecData[i] = pos(i);
      for(int i = 0; i < dof; i++) OP->NewVelocityVector->VecData[i] = 0;
      for(int i = 0; i < dof; i++) OP->NewAccelerationVector->VecData[i] = 0;
