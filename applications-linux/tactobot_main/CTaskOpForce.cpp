...
namespace scl_app
{
  ...
  // Inherited stuff
  //************************
  bool CTaskOpForce::init(STaskBase* arg_task_data,
      CDynamicsBase* arg_dynamics)
  {
    try
    {
      ...
      data_ = dynamic_cast<STaskOpForce*>(arg_task_data);
    ...
    {
      std::cerr<<"\nCTaskOpForce::init() :"<<e.what();
      has_been_init_ = false;
    }
    return has_been_init_;
  }

  STaskBase* CTaskOpForce::getTaskData()
  { return data_; }

  /** Sets the current goal position */
  bool CTaskOpForce::setGoalForce(const Eigen::VectorXd & arg_goal)
  {
    ...
    {
      data_->force_goal_ = arg_goal;
      return true;
    }
#ifdef DEBUG
    else
    {
      std::cerr<<"\nCTaskOpForce::setGoalForce() : Error : Goal vector's size != data_->dof_task_"<<std::flush;
      assert(false);
    }
#endif
    return false;
  ...
  bool CTaskOpForce::computeServo(const SRobotSensors* arg_sensors)
  {
    bool flag = true;
#ifdef DEBUG
    ...
    if(data_->has_been_init_)
    {
      ...
      Ks.setIdentity(data_->dof_task_,data_->dof_task_);

      //Transformation  for forces from end effector frame to ground frame
      Eigen::MatrixXd R_tilde(6,6);
      Eigen::Matrix3d R_o_lnk;
      Eigen::Matrix3d zeroM;
      zeroM.setZero();
      //TODO: Make the end effector frame same as the force sensor frame so things are consistent. Find
      //the corresponding transformation
      R_o_lnk = data_->rbd_->T_o_lnk_.rotation();
      R_tilde<<R_o_lnk,zeroM,zeroM,R_o_lnk;
//      R_tilde.setIdentity();


      //Compute the servo torques
      //force measured in EF frame
      std::cout << "kp: " << data_->kp_.rows() << " " << data_->kp_.cols() << "\n";
      std::cout << "force: " << data_->force_.rows() << " " << data_->force_.cols() << "\n";

      tmp1 =  R_tilde * data_->kp_ * (data_->force_goal_-data_->force_);
      std::cout << "tmp1: " << tmp1.rows() << " " << tmp1.cols() << "\n";


      //Obtain force to be applied to a unit mass floating about

      data_->force_prime_ = tmp1;
      ...
      data_->force_prime_ = data_->force_prime_.array().min(data_->force_task_max_.array());//Min of self and max
      data_->force_prime_ = data_->force_prime_.array().max(data_->force_task_min_.array());//Max of self and min


      if(data_->flag_compute_op_inertia_)
      { data_->force_task_ = R_tilde * data_->force_goal_ + Ks.inverse() * data_->M_task_ * data_->force_prime_;  }
      else
      { data_->force_task_ = R_tilde * data_->force_goal_ + Ks.inverse()  * data_->force_prime_; }
      ...
      data_->force_task_ = data_->Omega_ * data_->force_task_;
  ...
  bool CTaskOpForce::computeModel(const SRobotSensors* arg_sensors)
  {
#ifdef DEBUG
    assert(has_been_init_);
    ...
    if(data_->has_been_init_)
    {
      ...
      data_->J_ = data_->J_6_.block(0,0,6,data_->robot_->dof_);
      ...
      if(use_svd_for_lambda_inv_)
      {
        ...
        if(0 < rank_loss)
        { std::cout<<"\nCTaskOpForce::computeModel() : Warning. Lambda_inv is ill conditioned. SVD rank loss (@.005) = "<<rank_loss; }
  ...
  bool CTaskOpForce::achievedGoalOri()
  {
    sFloat dist;
    dist = fabs((data_->force_-data_->force_goal_).norm());
  ...
  scl::sBool registerType_TaskOpForce()
  {
    ...
    try
    {
      sutil::CDynamicType<std::string,scl_app::CTaskOpForce> typeCTaskOpForce(std::string("CTaskOpForce"));
      flag = typeCTaskOpForce.registerType();
      if(false == flag) {throw(std::runtime_error("Could not register type CTaskOpForce"));}

      sutil::CDynamicType<std::string,scl_app::STaskOpForce> typeSTaskOpForce(std::string("STaskOpForce"));
      flag = typeSTaskOpForce.registerType();
      if(false == flag) {throw(std::runtime_error("Could not register type STaskOpForce"));}

#ifdef DEBUG
      std::cout<<"\nregisterType_TaskOpForce() : Registered my cool force control task with the database";
#endif
    }
    catch (std::exception& e)
    {
      std::cout<<"\nregisterType_TaskOpForce() : Error : "<<e.what();
      return false;
    }
    return true;
  }
