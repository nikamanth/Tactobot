...
namespace scl_app
{
  ...
  bool CTaskOpOri::computeServo(const SRobotSensors* arg_sensors)
  {
    ...
    if(data_->has_been_init_)
    {
      ...
      //block(0,0,3,3);
      //   R_on(0,0)=T_on(0,0);   R_on(0,1)=T_on(0,1);   R_on(0,2)=T_on(0,2);
      //   R_on(1,0)=T_on(1,0);   R_on(1,1)=T_on(1,1);   R_on(1,2)=T_on(1,2);
      //   R_on(2,0)=T_on(2,0);   R_on(2,1)=T_on(2,1);   R_on(2,2)=T_on(2,2);

      //   Eigen::VectorXd temp_lam,lam_prs; temp_lam.setZero(4);lam_prs.setZero(4);
      //   temp_lam(3)=0.5*pow(1+R_on(0,0)+R_on(1,1)+R_on(2,2),0.5) //see function for sqrt
      //   temp_lam(0)=0.5*pow(1+R_on(0,0)-R_on(1,1)-R_on(2,2),0.5) //see function for sqrt
      //   temp_lam(1)=0.5*pow(1-R_on(0,0)+R_on(1,1)-R_on(2,2),0.5) //see function for sqrt
      //   temp_lam(2)=0.5*pow(1-R_on(0,0)-R_on(1,1)+R_on(2,2),0.5) //see function for sqrt
      //
      //   if(temp_lam(0)!=0.0) {
      //	   lam_prs(3)=temp_lam(3);
      //	   lam_prs(2)=(R_on(2,1)-R_on(1,2))/(4*temp_lam(3));
      //	   lam_prs(1)=(R_on(1,3)-R_on(3,1))/(4*temp_lam(3));
      //	   lam_prs(0)=(R_on(3,2)-R_on(2,3))/(4*temp_lam(3));
      //   }
      //   else  {
      //	   lam_prs(0)=temp_lam(0);
      //	   lam_prs(1)=(R_on(2,1)+R_on(1,2))/(4*temp_lam(0));
      //	   lam_prs(2)=(R_on(3,1)+R_on(1,3))/(4*temp_lam(0));
      //	   lam_prs(3)=(R_on(3,2)-R_on(2,3))/(4*temp_lam(0));
      //
      //   }
