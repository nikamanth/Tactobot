...
#define SCL_COPPTTASK_TASK_DOF 6

  STaskOpForce::STaskOpForce() : STaskBase(),
      link_name_(""),
      link_ds_(S_NULL),
  ...
  STaskOpForce::~STaskOpForce()
  { }


  bool STaskOpForce::initTaskParams()
  {
    try
    {
      if(6!=dof_task_)//This is a end effector force & moment based op point task
      { throw(std::runtime_error("Operational point tasks MUST have 6 dofs (xyz rotation at a point)."));  }
      ...
      force_goal_.setZero(dof_task_);
      force_.setZero(dof_task_);



      force_task_.setZero(dof_task_);
      Omega_.setIdentity(dof_task_,dof_task_);
    }
    catch(std::exception& e)
    {
      std::cerr<<"\nSTaskOpForce::init() : "<<e.what();
      return false;
    }
    return true;
  }
