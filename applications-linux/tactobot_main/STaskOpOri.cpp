...
#define SCL_COPPTTASK_TASK_DOF 3
  ...
  STaskOpOri::STaskOpOri() : STaskBase(),
      link_name_(""),
      link_ds_(S_NULL),
  ...
  STaskOpOri::~STaskOpOri()
  { }


  bool STaskOpOri::initTaskParams()
  {
    try
    {
      if(3!=dof_task_)//This is a position based op point task
      { throw(std::runtime_error("Operational point tasks MUST have 3 dofs (xyz rotation at a point)."));  }
      ...
      lam_ = Eigen::Quaterniond::Identity();
      dlam_.setZero(dof_task_);
      ddlam_.setZero(dof_task_);
      delta_phi_.setZero(dof_task_);
      omega_inp.setIdentity(dof_task_,dof_task_); 	

      lam_goal_= Eigen::Quaterniond::Identity();
      dlam_goal_.setZero(dof_task_);
      ddlam_goal_.setZero(dof_task_);
