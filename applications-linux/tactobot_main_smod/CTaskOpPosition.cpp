...
namespace scl_app
{
  ...
  CTaskOpPosition::CTaskOpPosition() :
      CTaskBase(),
      data_(S_NULL),
  ...
  // Inherited stuff
  //************************
  bool CTaskOpPosition::init(STaskBase* arg_task_data,
      CDynamicsBase* arg_dynamics)
  {
    try
    {
      ...
      data_ = dynamic_cast<STaskOpPosition*>(arg_task_data);
    ...
    {
      std::cerr<<"\nCTaskOpPosition::init() :"<<e.what();
      has_been_init_ = false;
    }
    return has_been_init_;
  }

  STaskBase* CTaskOpPosition::getTaskData()
  { return data_; }

  /** Sets the current goal position */
  bool CTaskOpPosition::setGoalPosition(const Eigen::VectorXd & arg_goal)
  {
    ...
    {
      std::cerr<<"\nCTaskOpPosition::setGoalPosition() : Error : Goal vector's size != data_->dof_task_"<<std::flush;
      assert(false);
    }
#endif
    return false;
  ...
  bool CTaskOpPosition::setGoalVel(const Eigen::VectorXd & arg_goal)
  {
    if((data_->dof_task_ == arg_goal.cols() && 1 == arg_goal.rows()) ||
        (1 == arg_goal.cols() && data_->dof_task_ == arg_goal.rows()) )
    {
    ...
    {
      std::cerr<<"\nCTaskOpPosition::setGoalVel() : Error : Goal vector's size != data_->dof_task_"<<std::flush;
      assert(false);
    }
#endif
    return false;
...
bool CTaskOpPosition::computeServo(const SRobotSensors* arg_sensors)
{
  bool flag = true;
#ifdef DEBUG
  ...
  if(data_->has_been_init_)
  {
    ...
    data_->ddx_ = data_->omega_inp * data_->ddx_; 	
    ...
    data_->force_task_(2) += data_->mass*9.81;
...
bool CTaskOpPosition::achievedGoalPosition()
{
  sFloat dist;
 ...
 /*******************************************
   ...
   *******************************************/
  scl::sBool registerType_TaskOpPosition()
  {
    bool flag;
    try
    {
      sutil::CDynamicType<std::string,scl_app::CTaskOpPosition> typeCTaskOpPosition(std::string("CTaskOpPosition"));
      flag = typeCTaskOpPosition.registerType();
      if(false == flag) {throw(std::runtime_error("Could not register type CTaskOpPosition"));}

      sutil::CDynamicType<std::string,scl_app::STaskOpPosition> typeSTaskOpPosition(std::string("STaskOpPosition"));
      flag = typeSTaskOpPosition.registerType();
      if(false == flag) {throw(std::runtime_error("Could not register type STaskOpPosition"));}

#ifdef DEBUG
      std::cout<<"\nregisterType_TaskOpPosition() : Registered my cool position control task with the database";
#endif
    }
    catch (std::exception& e)
    {
      std::cout<<"\nregisterType_TaskOpPosition() : Error : "<<e.what();
      return false;
    }
    return true;
  }
